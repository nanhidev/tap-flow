...
viewEventReportPage.clicksOnButton(button);
}
...
viewEventReportPage.isUrlCorrect();
}
...
viewEventReportPage.isPageLoaded();
}
...
viewEventReportPage.applyMultipleFilters();
}
...
viewEventReportPage.isExportedDataAccurate();
}
...
// DUPLICATE: @Then("the candidates should be sorted correctly based on the {string}")
// DUPLICATE: public void the_candidates_should_be_sorted_correctly_based_on_the_new(String newSortingCriterion) {
// DUPLICATE: viewEventReportPage.isCandidatesSortedCorrectly(newSortingCriterion);
// DUPLICATE: }
...
viewEventReportPage.areCommentsMatchingExpected();
}
...
viewEventReportPage.noFormattingIssuesPresent();
}
...
viewEventReportPage.validateSummaryReadability();
}
...
viewEventReportPage.checkForDiscrepancies();
}
...
@Then("no discrepancies are found between the summary and detailed data")
public void no_discrepancies_are_found_between_the_summary_and_detailed_data() {
viewEventReportPage.noDiscrepanciesFound();
}
...
@Then("the event report should be displayed")
public void the_event_report_should_be_displayed() {
viewEventReportPage.isEventReportDisplayed(), "Event report is not displayed.";
}
}