...
@When("the sorting options are visible and accessible")
public void the_sorting_options_are_visible_and_accessible() {
...
public void the_event_card_displays_clearly(String eventCardMessage) {
Assert.assertEquals(eventCardMessage, viewEventStatusPage.getEventCardDisplay());
}
...
viewEventStatusPage.hasBrowserSpecificIssues();
}
...
viewEventStatusPage.hasConfusionFromRecurringEvents();
}
...
viewEventStatusPage.hasLanguageConfusion();
}

@Given("the user checks the labels displayed for the event")
...
viewEventStatusPage.filterEventsByStatus();
}
...
viewEventStatusPage.isStatusAccurateWithoutDelay();
}
...
public void the_user_navigates_to_the_events_section_on(String platform) {
viewEventStatusPage.navigateToEventsSection(platform);
}
}