...
viewEventStatusPage.observeCancelledEventCard();
}
...
// DUPLICATE: @When("the user locates an event that has a status of {string}")
public void the_user_locates_an_event_that_has_a_status_of_not_started(String eventStatus) {
viewEventStatusPage.locateEventByStatus(eventStatus);
}
@Then("the event card should display {string}")
public void the_event_card_should_display_event_not_started_yet(String message) {
...
viewEventStatusPage.isMobileLayoutAccessible();
}
...
viewEventStatusPage.isLanguageDistinct();
}

@Given("the user navigates to the events section on {string}")
public void the_user_navigates_to_the_events_section_on(String url) {
driver.get(url);
viewEventStatusPage.navigateToEventsSection();
}
...
viewEventStatusPage.areEventLabelsDisplayed();
}
@Then("the event card displays {string} clearly")
public void the_event_card_displays_clearly(String expectedText) {
Assert.assertEquals(expectedText, viewEventStatusPage.getEventCardText());
}
...
viewEventStatusPage.areAllOccurrencesStatusDisplayed();
}
@Then("the cancelled event is visible in the list of events on {string}")
public void the_cancelled_event_is_visible_in_the_list_of_events_on(String date) {
viewEventStatusPage.isCancelledEventVisible(date);
}
...
viewEventStatusPage.findNotStartedEvent();
}
...
viewEventStatusPage.isRecurringEventClear();
}
@Then("the event card displays event cancelled clearly on {string}")
public void the_event_card_displays_event_cancelled_clearly_on(String date) {
viewEventStatusPage.isEventCancelledDisplayed(date);
}
@Then("no errors occur during the sorting process")
public void no_errors_occur_during_the_sorting_process() {
viewEventStatusPage.isSortingErrorOccurred();
}
...
@When("the user locates an event with multiple participants")
public void the_user_locates_an_event_with_multiple_participants() {
viewEventStatusPage.findEventWithMultipleParticipants();
}
@When("the user locates an event with a status of cancelled")
public void the_user_locates_an_event_with_a_status_of_cancelled() {
viewEventStatusPage.findCancelledEvent();
}
@Then("the event card should display event not started yet")
public void the_event_card_should_display_event_not_started_yet() {
viewEventStatusPage.isEventNotStartedYetDisplayed();
}
@Then("locates the filtering options available")
public void locates_the_filtering_options_available() {
viewEventStatusPage.areFilteringOptionsAvailable();
}
