...
viewJobEventsPage.login("user1", "pass1");
}
...
viewJobEventsPage.selectSpecificJobOpeningCard();
}
...
viewJobEventsPage.clickEventsButton();
}
...
viewJobEventsPage.clickButton(buttonName);
}
...
viewJobEventsPage.doAllEventCardsReflectAccurateInformation();
}
@When("the user observes the order of event cards displayed")
public void the_user_observes_the_order_of_event_cards_displayed() {
viewJobEventsPage.observeOrderOfEventCards();
}
...
viewJobEventsPage.clickBackButton();
}
...
viewJobEventsPage.observeTransitionUI();
}
...
@When("the recruiter clicks the refresh button on the browser")
public void the_recruiter_clicks_the_refresh_button_on_the_browser() {
viewJobEventsPage.clickRefreshButton();
}
@Then("the events page reloads correctly")
public void the_events_page_reloads_correctly() {
viewJobEventsPage.isEventsPageReloadedCorrectly();
}
...
viewJobEventsPage.areEventsConsistentBeforeAndAfterRefresh();
}
...
viewJobEventsPage.clickEventsButtonForJobOpening();
}
// DUPLICATE: @When("the user waits for the events page to load")
// DUPLICATE: public void the_user_waits_for_the_events_page_to_load() {
// DUPLICATE: viewJobEventsPage.waitForEventsPageToLoad();
// DUPLICATE: }
...
@Then("the user observes the events page reload")
public void the_user_observes_the_events_page_reload() {
